// Testcase1: Minimal 1x1x1x1 -> 1x1x1x1 convolution (is_optimize=true)
// Using is_optimize=true to avoid RowMajor2Row16x4MajorInt8 stack overflow
...
// Testcase7: Minimal 1x1x1x1 -> 1x1x1x1 convolution (is_optimize=false)
// Non-optimized path test
TEST_F(ConvInt8Test, ConvInt8_minimal_1x1_no_opt) {
  // Input: batch=1, h=1, w=1, c=1
  std::vector<int8_t> input = {10};
  ...
  // Buffer sizes for is_optimize=false
  int kernel_plane = 1 * 1;  // = 1
  int deep = kernel_plane * 1;  // = 1
#ifdef ENABLE_ARM32
  int unit_size = 16;   // UP_ROUND(deep, 16)
  int up_round_oc = 2;  // UP_ROUND(1, 2)
#else
  int unit_size = 16;   // UP_ROUND(deep, 16) = 16 (non-opt uses C16NUM)
  int up_round_oc = 4;  // UP_ROUND(1, 4) = 4 (non-opt uses C4NUM)
#endif
  int tile_n = 1;
  int input_sum_size = tile_n * up_round_oc;
  ...
  MATMUL_OPT_R_FUNC matmul_func = nullptr;  // Not used in non-optimize path

  ConvInt8(input.data(), packed_input.data(), matmul_input.data(), packed_weight.data(),
           bias_data.data(), output.data(), filter_zp.data(), input_sum.data(),
           task_id, &conv_param, matmul_func, is_optimize);

  std::cout << "ConvInt8Test-ConvInt8_minimal_1x1_no_opt output:\n";
  std::for_each(output.begin(), output.end(), [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << "\nConvInt8Test-ConvInt8_minimal_1x1_no_opt benchmark:\n";
  std::for_each(benchmark.begin(), benchmark.end(),
                [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << std::endl;
...
// Testcase8: 2x2 spatial input with 1x1 kernel (is_optimize=false)
// Input: 1x2x2x1, Output: 1x2x2x1
TEST_F(ConvInt8Test, ConvInt8_2x2_spatial_no_opt) {
  // Input: batch=1, h=2, w=2, c=1
  std::vector<int8_t> input = {1, 2, 3, 4};
  ...
  int up_round_oc = 2;
#else
  int unit_size = 16;  // non-opt uses C16NUM
  int up_round_oc = 4;  // non-opt uses C4NUM
#endif
  int tile_n = 4;
  int input_sum_size = tile_n * up_round_oc;
  ...
  std::cout << "ConvInt8Test-ConvInt8_2x2_spatial_no_opt output:\n";
  std::for_each(output.begin(), output.end(), [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << "\nConvInt8Test-ConvInt8_2x2_spatial_no_opt benchmark:\n";
  std::for_each(benchmark.begin(), benchmark.end(),
                [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << std::endl;
...
// Testcase9: With bias test (is_optimize=false)
// Input: 1x1x1x1, Weight: 2, Bias: 5 -> Output: 10*2+5 = 25
TEST_F(ConvInt8Test, ConvInt8_with_bias_no_opt) {
  std::vector<int8_t> input = {10};
  std::vector<int8_t> benchmark = {25};
  ...
  std::cout << "ConvInt8Test-ConvInt8_with_bias_no_opt output: " << static_cast<int32_t>(output[0]) << std::endl;
  std::cout << "ConvInt8Test-ConvInt8_with_bias_no_opt benchmark: " << static_cast<int32_t>(benchmark[0]) << std::endl;

  float similarity = get_cosine_similarity_int8(output.data(), benchmark.data(), output.size());
  ASSERT_GT(similarity, accuracy_threshold);
}

// Testcase10: Two output channels (is_optimize=false)
// Input: 1x1x1x1 -> Output: 1x1x1x2
TEST_F(ConvInt8Test, ConvInt8_two_output_channels_no_opt) {
  std::vector<int8_t> input = {10};
  std::vector<int8_t> benchmark = {10, 20};  // 10*1, 10*2
...
#else
  int unit_size = 16;  // non-opt uses C16NUM
  int up_round_oc = 4;  // UP_ROUND(2, 4) = 4 (non-opt uses C4NUM)
#endif
  int tile_n = 1;
  int input_sum_size = tile_n * up_round_oc;
  ...
  // Weight for channel 0 = 1, channel 1 = 2
  packed_weight[0] = 1;   // channel 0
  packed_weight[16] = 2;   // channel 1 (offset by unit_size)
  ...
  std::cout << "ConvInt8Test-ConvInt8_two_output_channels_no_opt output:\n";
  std::for_each(output.begin(), output.end(), [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << "\nConvInt8Test-ConvInt8_two_output_channels_no_opt benchmark:\n";
  std::for_each(benchmark.begin(), benchmark.end(),
                [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << std::endl;
...
// Testcase11: Activation clipping test (is_optimize=false)
// Input: 100, Weight: 2 -> Result 200 should be clipped to 127
TEST_F(ConvInt8Test, ConvInt8_activation_clip_no_opt) {
  std::vector<int8_t> input = {100};
  std::vector<int8_t> benchmark = {127};  // 100*2=200, clipped to 127
  ...
  std::cout << "ConvInt8Test-ConvInt8_activation_clip_no_opt output: " << static_cast<int32_t>(output[0]) << std::endl;
  std::cout << "ConvInt8Test-ConvInt8_activation_clip_no_opt benchmark: " << static_cast<int32_t>(benchmark[0]) << std::endl;

  float similarity = get_cosine_similarity_int8(output.data(), benchmark.data(), output.size());
  ASSERT_GT(similarity, accuracy_threshold);
}

// Testcase12: Multiple input channels (is_optimize=false)
// Input: 1x1x1x2 -> Output: 1x1x1x1
TEST_F(ConvInt8Test, ConvInt8_multi_input_channel_no_opt) {
  // Input: 2 channels
  std::vector<int8_t> input = {10, 20};
  ...
  int deep = kernel_plane * 2;  // = 2 (2 input channels)
#ifdef ENABLE_ARM32
  int unit_size = 16;   // UP_ROUND(2, 16) = 16
  int up_round_oc = 2;
#else
  int unit_size = 16;   // UP_ROUND(2, 16) = 16 (non-opt uses C16NUM)
  int up_round_oc = 4;  // non-opt uses C4NUM
#endif
  int tile_n = 1;
  int input_sum_size = tile_n * up_round_oc;
  ...
  std::cout << "ConvInt8Test-ConvInt8_multi_input_channel_no_opt output: " << static_cast<int32_t>(output[0]) << std::endl;
  std::cout << "ConvInt8Test-ConvInt8_multi_input_channel_no_opt benchmark: " << static_cast<int32_t>(benchmark[0]) << std::endl;
