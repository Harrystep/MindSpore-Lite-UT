// Testcase1: Minimal 1x1x1x1 -> 1x1x1x1 convolution (is_optimize=false)
// Tests the simplest possible case to avoid ASAN heap-stack overflow
TEST_F(ConvInt8Test, ConvInt8_minimal_1x1_no_opt) {
  ...
  // ConvParameter setup - using stack allocation with small size
...
#ifdef ENABLE_ARM32
  int unit_size = 16;  // UP_ROUND(kernel_plane * input_channel, 16)
  int up_round_oc = 2;  // UP_ROUND(output_channel, 2)
#else
  int unit_size = 16;  // UP_ROUND(kernel_plane * input_channel, 16)
  int up_round_oc = 4;  // UP_ROUND(output_channel, 4)
#endif
  int tile_n = 1;
  int input_sum_size = tile_n * up_round_oc;

  // Allocate buffers on heap via vector to avoid stack overflow
  std::vector<int8_t> packed_input(unit_size * tile_n, 0);
  std::vector<int8_t> matmul_input(kernel_plane * 16 * tile_n, 0);
...
// Testcase2: 2x2 spatial input with 1x1 kernel (is_optimize=false)
...
// Testcase3: With bias test (is_optimize=false)
...
// Testcase4: Two output channels (is_optimize=false)
...
#else
  int unit_size = 16;
  int up_round_oc = 4;  // UP_ROUND(2, 4) = 4
#endif
  int tile_n = 1;
  int input_sum_size = tile_n * up_round_oc;
...
// Testcase5: Activation clipping test (is_optimize=false)
